...
<script>
  ...
  import tableOption from '@/const/groupManage/tableOption';
  import { parseTime } from '@/filters/'
  import { validatenull } from '@/util/validate';
  import { getGroupTableData,insertGroupData,updateGroupData,removeGroupData } from '@/api/group';
  export default {
    name: 'table',
    data () {
      return {
    ...
    methods: {
      ...
      handleList (form) {
        ...
        getGroupTableData(Object.assign({}, form, { page:  `${this.tablePage}` }))
          .then(res => {
            const data = res.data;
            setTimeout(() => {
       ...
       **/
      handleSave (row, done) {
        insertGroupData(Object.assign({}, row))
          .then(() => {
      ...
      handleDel (row, index) {
        this.$confirm(`是否确认删除序号为${row.name}`, '提示', {
          ...
          .then(() => {
            removeGroupData(Object.assign({}, row))
              .then(() => {
                setTimeout(() => {
      ...
      handleUpdate (row, index, done) {
        updateGroupData(Object.assign({}, row))
          .then(() => {
